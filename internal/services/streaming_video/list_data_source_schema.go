...
var _ datasource.DataSourceWithConfigValidators = (*StreamingVideosDataSource)(nil)

func ListDataSourceSchema(ctx context.Context) schema.Schema {
	return schema.Schema{
		Attributes: map[string]schema.Attribute{
			"client_user_id": schema.Int64Attribute{
				Description: "Find videos where \"`client_user_id`\" meta field is equal to the search value",
				Optional:    true,
			},
			"fields": schema.StringAttribute{
				Description: "Restriction to return only the specified attributes, instead of the entire dataset. Specify, if you need to get short response. The following fields are available for specifying: id, name, duration, status, `created_at`, `updated_at`, `hls_url`, screenshots, `converted_videos`, priority, `stream_id`. Example, ?fields=id,name,`hls_url`",
				Optional:    true,
			},
			"id": schema.StringAttribute{
				Description: "IDs of the videos to find. You can specify one or more identifiers separated by commas. Example, ?id=1,101,1001",
				Optional:    true,
			},
			"per_page": schema.Int64Attribute{
				Description: "Items per page number. Use it to list the paginated content",
				Optional:    true,
			},
			"search": schema.StringAttribute{
				Description: "Aggregated search condition. If set, the video list is filtered by one combined SQL criterion:\n- id={s} OR slug={s} OR name like {s}\ni.e. \"/videos?search=1000\" returns list of videos where id=1000 or slug=1000 or name contains \"1000\".",
				Optional:    true,
			},
			"status": schema.StringAttribute{
				Description: "Use it to get videos filtered by their status. Possible values:\n- empty\n- pending\n- viewable\n- ready\n- error",
				Optional:    true,
			},
			"stream_id": schema.Int64Attribute{
				Description: "Find videos recorded from a specific stream, so for which \"`stream_id`\" field is equal to the search value",
				Optional:    true,
			},
			...
			"items": schema.ListNestedAttribute{
				...
				Computed:    true,
				CustomType:  customfield.NewNestedObjectListType[StreamingVideosItemsDataSourceModel](ctx),
				NestedObject: schema.NestedAttributeObject{
					Attributes: map[string]schema.Attribute{
						...
						"converted_videos": schema.ListNestedAttribute{
							...
							Computed:    true,
							CustomType:  customfield.NewNestedObjectListType[StreamingVideosConvertedVideosDataSourceModel](ctx),
							NestedObject: schema.NestedAttributeObject{
								Attributes: map[string]schema.Attribute{
									"id": schema.Int64Attribute{
...
func (d *StreamingVideosDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = ListDataSourceSchema(ctx)
}

func (d *StreamingVideosDataSource) ConfigValidators(_ context.Context) []datasource.ConfigValidator {
