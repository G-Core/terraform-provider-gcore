...
var _ datasource.DataSourceWithConfigure = (*DNSZonesDataSource)(nil)
...
func (d *DNSZonesDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	env := DNSZonesZonesListDataSourceEnvelope{}
	maxItems := int(data.MaxItems.ValueInt64())
	acc := []attr.Value{}
	if maxItems <= 0 {
		maxItems = 1000
	}
	page, err := d.client.DNS.Zones.List(ctx, params)
	if err != nil {
		resp.Diagnostics.AddError("failed to make http request", err.Error())
		return
	}

	for page != nil && len(page.Zones) > 0 {
		...
		acc = append(acc, env.Zones.Elements()...)
		if len(acc) >= maxItems {
			break
		}
	...
	result, diags := customfield.NewObjectListFromAttributes[DNSZonesItemsDataSourceModel](ctx, acc)
	resp.Diagnostics.Append(diags...)
	data.Items = result
