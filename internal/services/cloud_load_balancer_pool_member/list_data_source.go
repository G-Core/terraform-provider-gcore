...
func (d *CloudLoadBalancerPoolMembersDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_cloud_load_balancer_pool_members"
}

func (d *CloudLoadBalancerPoolMembersDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *CloudLoadBalancerPoolMembersDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data *CloudLoadBalancerPoolMembersDataSourceModel
	...
	env := CloudLoadBalancerPoolMembersResultsListDataSourceEnvelope{}
	maxItems := int(data.MaxItems.ValueInt64())
	acc := []attr.Value{}
	if maxItems <= 0 {
		maxItems = 1000
	}
	page, err := d.client.Cloud.LoadBalancers.Pools.Members.List(
		ctx,
	...
	result, diags := customfield.NewObjectListFromAttributes[CloudLoadBalancerPoolMembersItemsDataSourceModel](ctx, acc)
	resp.Diagnostics.Append(diags...)
	data.Items = result
