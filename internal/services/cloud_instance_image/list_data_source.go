...
func (d *CloudInstanceImagesDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *CloudInstanceImagesDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data *CloudInstanceImagesDataSourceModel
	...
	env := CloudInstanceImagesResultsListDataSourceEnvelope{}
	maxItems := int(data.MaxItems.ValueInt64())
	acc := []attr.Value{}
	if maxItems <= 0 {
		maxItems = 1000
	}
	page, err := d.client.Cloud.Instances.Images.List(ctx, params)
	if err != nil {
	...
	result, diags := customfield.NewObjectListFromAttributes[CloudInstanceImagesItemsDataSourceModel](ctx, acc)
	resp.Diagnostics.Append(diags...)
	data.Items = result
