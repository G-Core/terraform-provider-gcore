...
type WaapDomainCustomRuleDataSourceModel struct {
	...
	FindOneBy   *WaapDomainCustomRuleFindOneByDataSourceModel                               `tfsdk:"find_one_by"`
}
...
func (m *WaapDomainCustomRuleDataSourceModel) toListParams(_ context.Context) (params waap.DomainCustomRuleListParams, diags diag.Diagnostics) {
	params = waap.DomainCustomRuleListParams{
		DomainID: m.DomainID.ValueInt64(),
	}

	if !m.FindOneBy.Action.IsNull() {
		params.Action = waap.DomainCustomRuleListParamsAction(m.FindOneBy.Action.ValueString())
	}
	...
	if !m.FindOneBy.Ordering.IsNull() {
		params.Ordering = waap.DomainCustomRuleListParamsOrdering(m.FindOneBy.Ordering.ValueString())
	}
...
type WaapDomainCustomRuleFindOneByDataSourceModel struct {
