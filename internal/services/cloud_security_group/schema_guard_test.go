...
// TestCloudSecurityGroupResourceOmitsTagsV2 fails if the read-only tags_v2
...
// of guard.
func TestCloudSecurityGroupResourceOmitsTagsV2(t *testing.T) {
	t.Parallel()

	if _, found := cloud_security_group.ResourceSchema(context.TODO()).Attributes["tags_v2"]; found {
		t.Error("tags_v2 is present in the gcore_cloud_security_group resource schema; " +
			"the read-only tag list belongs on the data sources, as `tags`")
	}
}

// TestCloudSecurityGroupResourceTagsStayWritable fails if `tags` on the resource
...
func TestCloudSecurityGroupResourceTagsStayWritable(t *testing.T) {
	t.Parallel()

	attribute, found := cloud_security_group.ResourceSchema(context.TODO()).Attributes["tags"]
	if !found {
		t.Fatal("tags is missing from the gcore_cloud_security_group resource schema; " +
			"it is the map the caller writes and must stay")
	}

	tags, ok := attribute.(resourceschema.MapAttribute)
	if !ok {
		t.Fatalf("tags on the gcore_cloud_security_group resource is a %T, want "+
			"resourceschema.MapAttribute; a nested list here is the server-derived "+
			"tag mirror back under a new name", attribute)
	}

	if !tags.Optional {
		t.Error("tags on the gcore_cloud_security_group resource is not Optional; " +
			"the writable map has turned into a read-only one, which is the " +
			"server-derived mirror in the map's clothing")
	}
}

// TestCloudSecurityGroupDataSourceExposesTags fails if the read-only tag list
// stops being exposed as `tags` on the data source.
func TestCloudSecurityGroupDataSourceExposesTags(t *testing.T) {
	t.Parallel()

	attribute, found := cloud_security_group.DataSourceSchema(context.TODO()).Attributes["tags"]
	if !found {
		t.Fatal("tags is missing from the gcore_cloud_security_group data source schema")
	}
