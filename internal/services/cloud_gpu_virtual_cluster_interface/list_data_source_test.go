...
// Interfaces do not exist without a cluster, so this test creates one (its
// servers_settings always includes one "external" interface) and then reads
// the list data source (deferred via depends_on) to assert it is returned.
//
...
func TestAccCloudGPUVirtualClusterInterfacesDataSource_basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccCloudGPUVirtualClusterInterfacesDataSourceConfig(rName),
				ConfigStateChecks: []statecheck.StateCheck{
					statecheck.ExpectKnownValue("data.gcore_cloud_gpu_virtual_cluster_interfaces.test",
						tfjsonpath.New("items").AtSliceIndex(0).AtMapKey("port_id"), knownvalue.NotNull()),
					statecheck.ExpectKnownValue("data.gcore_cloud_gpu_virtual_cluster_interfaces.test",
						tfjsonpath.New("items").AtSliceIndex(0).AtMapKey("network_id"), knownvalue.NotNull()),
					statecheck.ExpectKnownValue("data.gcore_cloud_gpu_virtual_cluster_interfaces.test",
						tfjsonpath.New("items").AtSliceIndex(0).AtMapKey("mac_address"), knownvalue.NotNull()),
				},
			},
		},
	})
...
func testAccCloudGPUVirtualClusterInterfacesDataSourceConfig(name string) string {
	return fmt.Sprintf(`
...
data "gcore_cloud_gpu_virtual_cluster_interfaces" "test" {
  project_id = %[1]s
  region_id  = %[2]s
