...
// Instance flavors are region-global and always present, so the list data
// source is read directly without creating any resource.

func TestAccCloudInstanceFlavorsDataSource_basic(t *testing.T) {
	resource.ParallelTest(t, resource.TestCase{
		PreCheck:                 func() { acctest.PreCheck(t) },
		ProtoV6ProviderFactories: acctest.ProtoV6ProviderFactories,
		Steps: []resource.TestStep{
			{
				Config: testAccCloudInstanceFlavorsDataSourceConfig(),
				ConfigStateChecks: []statecheck.StateCheck{
					statecheck.ExpectKnownValue("data.gcore_cloud_instance_flavors.test",
...
func TestAccCloudInstanceFlavorsDataSource_maxItems(t *testing.T) {
	resource.ParallelTest(t, resource.TestCase{
		PreCheck:                 func() { acctest.PreCheck(t) },
		ProtoV6ProviderFactories: acctest.ProtoV6ProviderFactories,
		Steps: []resource.TestStep{
			{
				Config: testAccCloudInstanceFlavorsDataSourceConfigMaxItems(1),
				ConfigStateChecks: []statecheck.StateCheck{
					statecheck.ExpectKnownValue("data.gcore_cloud_instance_flavors.test",
...
func testAccCloudInstanceFlavorsDataSourceConfig() string {
	return fmt.Sprintf(`
data "gcore_cloud_instance_flavors" "test" {
  project_id = %[1]s
  region_id  = %[2]s
}
...
func testAccCloudInstanceFlavorsDataSourceConfigMaxItems(n int) string {
	return fmt.Sprintf(`
data "gcore_cloud_instance_flavors" "test" {
  project_id = %[1]s
  region_id  = %[2]s
