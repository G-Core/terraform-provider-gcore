terraform {
  required_providers {
    gcore = {
      source  = "G-Core/gcore"
      version = "~> 0.9"
    }
  }
}

provider "gcore" {
  permanent_api_token = var.api_token
}

variable "api_token" {
  type      = string
  sensitive = true
}

resource "gcore_cdn_cacert" "test" {
  name = "tf-test-cacert-jira-22617"
  cert = <<-EOT
-----BEGIN CERTIFICATE-----

OK79sVMVjRGHAEhkkCz2uULKjDpjfb12QW45cKEo/Hce
-----END CERTIFICATE-----
EOT
}

output "cert_id" {
  value = gcore_cdn_cacert.test.id
}

output "cert_name" {
  value = gcore_cdn_cacert.test.name
}

output "has_related_resources" {
  value = gcore_cdn_cacert.test.has_related_resources
}
