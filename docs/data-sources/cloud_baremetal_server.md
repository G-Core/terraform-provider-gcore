...
page_title: "gcore_cloud_baremetal_server Data Source - Gcore"
subcategory: ""
description: |-
...
# gcore_cloud_baremetal_server (Data Source)
...
```terraform
data "gcore_cloud_baremetal_server" "example_cloud_baremetal_server" {
  project_id = 1
  region_id = 1
...
- `ddos_profile` (Attributes) Bare metal advanced DDoS protection profile. It is always `null` if query parameter `with_ddos=true` is not set. (see [below for nested schema](#nestedatt--ddos_profile))
...
- `flavor` (Attributes) Flavor details (see [below for nested schema](#nestedatt--flavor))
- `id` (String) Server ID
