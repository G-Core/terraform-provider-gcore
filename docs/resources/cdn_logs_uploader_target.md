...
- `bucket_name` (String) Name of the Gcore Object Storage bucket. With `storage_id`, the bucket must already exist in the storage.

Optional:

- `access_key_id` (String) Access key ID for the Gcore Object Storage. Required when `storage_id` is not set.
- `directory` (String) Directory in the bucket where logs will be uploaded.
