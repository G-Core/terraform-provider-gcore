...
- `healthmonitor` (Attributes) New pool health monitor settings (see [below for nested schema](#nestedatt--healthmonitor))
- `lb_algorithm` (String) New load balancer pool algorithm of how to distribute requests
...
- `members` (Attributes List) New sequence of load balancer pool members. If members are the same (by address + port), they will be kept as is without recreation and downtime. (see [below for nested schema](#nestedatt--members))
- `name` (String) New pool name
- `project_id` (Number) Project ID
- `protocol` (String) New communication protocol
...
- `session_persistence` (Attributes) New session persistence settings (see [below for nested schema](#nestedatt--session_persistence))
