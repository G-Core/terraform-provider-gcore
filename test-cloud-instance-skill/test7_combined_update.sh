...
echo "============================================"
echo "TEST 7: Combined Update (name + flavor)"
echo "============================================"
echo ""
echo "Current state:"
echo "  Name: test-instance-skill"
echo "  Flavor: g1-standard-1-2"
echo ""
echo "Target state:"
echo "  Name: test-instance-skill-renamed"
echo "  Flavor: g1-standard-2-4"
echo ""
echo "=== Step 1: Terraform Plan ==="
terraform plan -out=tfplan 2>&1 | tee evidence/test7_plan.log

echo ""
echo "=== Step 2: Terraform Apply (Combined Update) ==="
terraform apply -auto-approve tfplan 2>&1 | tee evidence/test7_apply.log

echo ""
echo "=== Step 3: Verify Outputs ==="
terraform output 2>&1 | tee evidence/test7_outputs.log

echo ""
echo "=== Step 4: Drift Check (2nd Plan - MUST show no changes) ==="
set +e
terraform plan -detailed-exitcode 2>&1 | tee evidence/test7_drift.log
DRIFT_EXIT=$?
set -e
